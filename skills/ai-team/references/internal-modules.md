# AI TEAM Internal Modules

This repository keeps many workflow modules as sibling skill folders, but AI TEAM is the public entry surface.

Use this file as the internal module index.

Always apply `execution-policy.md` first so AI TEAM knows whether the project is `github_enabled` or `local_only`.

## Planning

- `../client-intake-normalizer/SKILL.md`
- `../solution-options-tradeoffs/SKILL.md`
- `../scope-convergence/SKILL.md`

## Orchestration

- `../ai-project-manager-orchestrator/SKILL.md`
- `../ai-team-planner/SKILL.md`
- `../backlog-management/SKILL.md`
- `../task-assignment-governance/SKILL.md`
- `../github-traceability-board-sync/SKILL.md`
- `../cross-agent-handover/SKILL.md`

## Implementation

- `../memory-bank/SKILL.md`
- `../repo-discovery/SKILL.md`
- `../project-developer/SKILL.md`
- `../api-contract-integration/SKILL.md`
- `../auth-identity/SKILL.md`
- `../frontend-ui-states/SKILL.md`
- `../infra-environments/SKILL.md`
- `../database-schema-migrations/SKILL.md`
- `../observability-monitoring/SKILL.md`

## Verification And Release

- `../review-verification/SKILL.md`
- `../qa-e2e-release/SKILL.md`
- `../security-production-readiness/SKILL.md`
- `../docs-sync-handover/SKILL.md`

## Delivery Oversight

- `../delivery-analytics-forecast/SKILL.md`

## GitHub Enablement

Load these only when `tracking_mode=github_enabled`:

- `../github-integration/SKILL.md`
- `../github-projects/SKILL.md`
- `../project-initialization/SKILL.md`

## Operational Guardrails

Use these scripts with AI TEAM when delivery moves from planning into disciplined execution:

- `../ai-team/scripts/summarize-memory-state.sh`
- `../ai-team/scripts/check-project-onboarding.sh`
- `../ai-team/scripts/validate-github-project-schema.sh`
- `../ai-team/scripts/validate-worker-ownership.sh`
- `../ai-team/scripts/check-memory-github-drift.sh`
- `../ai-team/scripts/ai-team-runner.sh`

## Legacy Or Supporting

- `../skill-router/SKILL.md`
- `../autonomous-agent/SKILL.md`
- `../project-manager/SKILL.md`
- `../project-initialization/SKILL.md`
- `../requirements-analysis/SKILL.md`
- `../timeline-roadmap/SKILL.md`
- `../team-setup/SKILL.md`
- `../team-roles/SKILL.md`
- `../github-integration/SKILL.md`
- `../github-projects/SKILL.md`
- `../docker-setup/SKILL.md`
- `../cicd-delivery/SKILL.md`
- `../coding-assistant/SKILL.md`
